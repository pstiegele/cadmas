...
module.exports = function (ws, callback) {
    var db = global.db;
    var query = "SELECT Activity.id,Activity.missionID, Activity.droneID, Activity.name, Activity.state, Activity.note, UNIX_TIMESTAMP(Activity.dt_created) AS dt_created, Activity.dt_ended, Activity.thumbnailpath, TIMESTAMPDIFF(MINUTE, Activity.dt_created, Activity.dt_ended) AS duration FROM Activity LEFT JOIN Drone ON Activity.droneID = Drone.id WHERE Drone.userID = ?";
